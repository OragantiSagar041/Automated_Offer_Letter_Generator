 ...
 */
export const generatePdfWithTemplate = async (htmlContent, templateUrl = '/Arah_Template.jpg') => {
    try {
        ...
        const CANVAS_SCALE = 2;   // 2x for crisp text
        ...
        if (isImage) {
            ...
            if (hdr[0] === 0x25 && hdr[1] === 0x50 && hdr[2] === 0x44 && hdr[3] === 0x46) {
                throw new Error(`File '${templateUrl}' is a PDF renamed as image. Convert it properly.`);
            }
            templateImage = imageBytes;
        } else {
            const pdfRes = await fetch(`${templateUrl}?t=${Date.now()}`);
            if (!pdfRes.ok) throw new Error(`Template PDF not found: ${templateUrl}`);
            templatePdfDoc = await PDFDocument.load(await pdfRes.arrayBuffer());
        }
